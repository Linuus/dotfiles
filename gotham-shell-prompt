...
function __promptline_ps1 {
  ...
  [ $is_prompt_empty -eq 1 ] && slice_prefix="$slice_empty_prefix"
  # section "z" slices
  __promptline_wrapper "Ruby: $(rbenv version-name)" "$slice_prefix" "$slice_suffix" && { slice_prefix="$slice_joiner"; is_prompt_empty=0; }
...
function __promptline_right_prompt {
  ...
  # section "z" slices
  __promptline_wrapper "Ruby: $(rbenv version-name)" "$slice_prefix" "$slice_suffix" && { slice_prefix="$slice_joiner"; }
...
function __promptline {
  ...
  local z_bg="${wrap}48;5;11${end_wrap}"
  local z_sep_fg="${wrap}38;5;11${end_wrap}"
  if [[ -n ${ZSH_VERSION-} ]]; then
